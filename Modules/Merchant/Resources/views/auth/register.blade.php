...
@section('main-content')
                ...
                <nav class="navbar navbar-expand-md py-1">
                    ...
                    <div class="collapse navbar-collapse" id="main-navigation">
                        <ul class="navbar-nav">
                            ...
                            <li class="nav-item">
                                <a href="javascript:;" title="Đăng ký thuê máy bán hàng" class="btn btn-primary btn-custom text-uppercase px-4 ">
                                    <strong>Đăng ký thuê máy bán hàng</strong>
                                </a>
                            </li>
                        </ul>
                    </div>
    ...
    <section class="bg-white">
        <div class="container">
            <div class="row">
                <div class="col-md-8 offset-md-2 pt-100 pb-50 form-machine">
                    @if ($errors->any())
                    ...
                    @endif
                    <form method="POST" action="{{ route('merchant.register') }}" class="user">
                        @csrf
                        <div class="form-group row">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <input type="text" class="pb-4 pt-4 form-control form-control-user" name="account" placeholder="{{__('Tài khoản đăng nhập')}}" value="{{ old('account') }}" required autofocus>
                            </div>
                            <div class="col-sm-6">
                                <input type="password" class="pb-4 pt-4 form-control form-control-user" name="password" placeholder="{{ __('Mật khẩu') }}" value="{{ old('password') }}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <input type="text" class="pb-4 pt-4 form-control form-control-user" name="merchant_name" placeholder="{{__('Tên hiển thị')}}" value="{{ old('merchant_name') }}" required>
                        </div>
                        <div class="form-group company-name">
                            <input type="text" class="pb-4 pt-4 form-control form-control-user" name="company_name" placeholder="{{__('Tên công ty/cá nhân')}}" value="{{ old('company_name') }}" required>
                        </div>
                        <div class="form-group">
                            <input type="text" class="pb-4 pt-4 form-control form-control-user" name="address" placeholder="{{__('Địa chỉ')}}" value="{{ old('address') }}">
                        </div>
                        <div class="form-group row">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <input type="email" class="pb-4 pt-4 form-control form-control-user" name="email" placeholder="{{ __('Email liên hệ') }}" value="{{ old('email') }}" required>
                            </div>
                            <div class="col-sm-6">
                                <input type="text" class="pb-4 pt-4 form-control form-control-user" name="phone" placeholder="{{ __('Số điện thoại liên hệ') }}" value="{{ old('phone') }}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <button type="submit" class="btn btn-primary btn-user btn-block btn-custom">
                                {{ __('Đăng ký người bán hàng') }}
                            </button>
                        </div>
                    </form>
                    <hr>
